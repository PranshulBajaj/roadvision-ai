...
function Reports() {
  ...
  const score = alerts.length === 0 ? 100 : Math.max(0, Math.round(100 - (drowsyCount * 15)))
    ...
    <div style={{ color: '#e2e8f0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
        {[
          ...
          { label: 'Alertness Score', value: `${score}/100`, color: score >= 80 ? '#22c55e' : score >= 50 ? '#f59e0b' : '#ef4444' },
        ].map(c => (
          <div key={c.label} style={card}>
            <div style={label2}>{c.label}</div>
            <div style={{ fontSize: '28px', fontWeight: '700', fontFamily: 'monospace', color: c.color }}>
              {c.value}
            </div>
          </div>
        ))}
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid #1e2535' }}>
              {['#', 'Type', 'EAR Value', 'MAR Value', 'Timestamp'].map(h => (
                <th key={h} style={{ padding: '10px', textAlign: 'left', color: '#64748b', fontWeight: '600' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {alerts.map((a, i) => (
              <tr key={a._id} style={{ borderBottom: '1px solid #1e2535' }}>
                ...
                <td style={{ padding: '10px', color: a.type === 'DROWSY' ? '#ef4444' : '#f59e0b', fontWeight: '600' }}>
                  {a.type === 'DROWSY' ? '😴' : '🥱'} {a.type}
                </td>
                <td style={{ padding: '10px', fontFamily: 'monospace' }}>{a.earValue}</td>
                <td style={{ padding: '10px', fontFamily: 'monospace' }}>{a.marValue}</td>
